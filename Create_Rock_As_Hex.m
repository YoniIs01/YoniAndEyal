function[ Rock_Matrix, Height, Width]=...
        Create_Rock_As_Hex(Num_Of_Grains_In_Row, Dolomite_Percentage)
...
%% Create a rock As Hexagon grid
if (mod(Num_Of_Grains_In_Row,2) == 0)
    Num_Of_Grains_In_Row = Num_Of_Grains_In_Row + 1;
end
figure('Visible','Off'); 
hold on;
r= sqrt(3) / 2;
[x y] = meshgrid(0:1:Num_Of_Grains_In_Row);
n = size(x,1);
x = r * x;
...
[X Y] = voronoi(x(:),y(:));
plot(X,Y,'r-')
axis([0 Num_Of_Grains_In_Row*r 0 Num_Of_Grains_In_Row*r]);
