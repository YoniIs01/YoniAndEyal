clear, close all, clc;
Rock_BW_Image=imread('2nd_image_of_cracks.tif');
for Decrease_factor=0.4:-0.05:0.05
    ...
    filename = strcat('Hor_HighDenseCracks',num2str(Decrease_factor),'density',num2str(round(maxdensity*100)),'%.tiff');
    imwrite(highdenseimage,filename);
    filename = strcat('Hor_LowDenseCracks',num2str(Decrease_factor),'density',num2str(round(mindensity*100)),'%.tiff');
